...
$softwareFilesDir = '../softwareFiles/';
if (!is_dir($softwareFilesDir)) {
    mkdir($softwareFilesDir, 0777, true);
}
...
if (isset($_POST['upload'])) {
    if (!empty($_FILES['file']['name'][0])) {
        ...
        for ($i = 0; $i < count($files['name']); $i++) {
            ...
            $targetFilePath = $softwareFilesDir . $fileName;
...
if (isset($_POST['delete']) && isset($_POST['file_name'])) {
    $fileToDelete = $softwareFilesDir . $_POST['file_name'];
...
if (isset($_POST['rename']) && isset($_POST['old_name']) && isset($_POST['new_name'])) {
    $oldPath = $softwareFilesDir . $_POST['old_name'];
    $newPath = $softwareFilesDir . $_POST['new_name'];
...
$filesAndFolders = array_diff(scandir($softwareFilesDir), array('.', '..'));
...
<div class="container mt-4">
    <div class="container-box">
        ...
        <ul class="nav nav-pills nav-fill bg-success p-2 rounded">
            <li class="nav-item">
                <a class="nav-link active btn btn-secondary shadow-lg" href="softwares.php">Softwares</a>
            </li>
            <li class="nav-item">
            ...
            <tbody>
                <?php foreach ($filteredFilesAndFolders as $item): ?>
                    <tr>
                        <td>
                            <?php
                            $itemPath = $softwareFilesDir . $item;
                            if (is_dir($itemPath)) {
