...
$ProductPreDir = '../productpresentations/';
if (!is_dir($ProductPreDir)) {
    mkdir($ProductPreDir, 0777, true);
}
...
if (isset($_POST['upload'])) {
    if (!empty($_FILES['file']['name'][0])) {
        ...
        for ($i = 0; $i < count($files['name']); $i++) {
            ...
            $targetFilePath = $$ProductPreDir . $fileName;
...
if (isset($_POST['delete']) && isset($_POST['file_name'])) {
    $fileToDelete = $ProductPreDir . $_POST['file_name'];
...
if (isset($_POST['rename']) && isset($_POST['old_name']) && isset($_POST['new_name'])) {
    $oldPath = $ProductPreDir . $_POST['old_name'];
    $newPath = $ProductPreDir . $_POST['new_name'];
...
$filesAndFolders = array_diff(scandir($ProductPreDir), array('.', '..'));
...
<div class="container mt-4">
    <div class="container-box">
        <h2 class="text-center text-dark">MANUAL FILES</h2>

        <!-- Navigation Tabs -->
        <ul class="nav nav-pills nav-fill bg-success p-2 rounded">
            <li class="nav-item">
                <a class="nav-link text-light" href="usermanual.php">User Manual</a>
            </li>
            <li class="nav-item">
                <a class="nav-link text-light" href="servicemanual.php">Service Manual</a>
            </li>
            <li class="nav-item">
                <a class="nav-link active btn btn-secondary" href="productpresentations.php">Product Presentations</a>
            </li>
            <li class="nav-item">
                <a class="nav-link text-light" href="videopresentations.php">Video Presentations</a>
            </li>
        </ul>
