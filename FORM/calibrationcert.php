...
$calibrationcertDir = '../calibrationcertFiles/';
if (!is_dir($calibrationcertDir)) {
    mkdir($calibrationcertDir, 0777, true);
}
...
if (isset($_POST['upload'])) {
    if (!empty($_FILES['file']['name'][0])) {
        ...
        for ($i = 0; $i < count($files['name']); $i++) {
            ...
            $targetFilePath = $calibrationcertDir . $fileName;
...
if (isset($_POST['delete']) && isset($_POST['file_name'])) {
    $fileToDelete = $calibrationcertDir . $_POST['file_name'];
...
if (isset($_POST['rename']) && isset($_POST['old_name']) && isset($_POST['new_name'])) {
    $oldPath = $calibrationcertDir . $_POST['old_name'];
    $newPath = $calibrationcertDir . $_POST['new_name'];
...
$filesAndFolders = array_diff(scandir($calibrationcertDir), array('.', '..'));
...
<div class="container mt-4">
    <div class="container-box">
        ...
        <ul class="nav nav-pills nav-fill bg-success p-2 rounded">
            <li class="nav-item">
                <a class="nav-link text-light" href="checklistPT-install.php">Checklist Pullout/Install</a>
            </li>
            <li class="nav-item">
                <a class="nav-link text-light" href="PMScert.php">PMS Certificates</a>
            </li>
            <li class="nav-item">
                <a class="nav-link active btn btn-secondary" href="calibrationcert.php">Calibration Certificates</a>
            </li>
            <li class="nav-item">
            ...
            <tbody>
                <?php foreach ($filteredFilesAndFolders as $item): ?>
                    <tr>
                        <td>
                            <?php
                            $itemPath = $calibrationcertDir . $item;
                            if (is_dir($itemPath)) {
