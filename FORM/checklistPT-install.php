...
$checklistPTinstallDir = '../checklistPT-installFiles/';
if (!is_dir($checklistPTinstallDir)) {
    mkdir($checklistPTinstallDir, 0777, true);
}
...
if (isset($_POST['upload'])) {
    if (!empty($_FILES['file']['name'][0])) {
        ...
        for ($i = 0; $i < count($files['name']); $i++) {
            ...
            $targetFilePath = $checklistPTinstallDir . $fileName;
...
if (isset($_POST['delete']) && isset($_POST['file_name'])) {
    $fileToDelete = $checklistPTinstallDir . $_POST['file_name'];
...
if (isset($_POST['rename']) && isset($_POST['old_name']) && isset($_POST['new_name'])) {
    $oldPath = $checklistPTinstallDir . $_POST['old_name'];
    $newPath = $checklistPTinstallDir . $_POST['new_name'];
...
$filesAndFolders = array_diff(scandir($checklistPTinstallDir), array('.', '..'));
...
<div class="container mt-4">
    <div class="container-box">
        <h2 class="text-center text-dark">FORM FILES</h2>

        <!-- Navigation Tabs -->
        <ul class="nav nav-pills nav-fill bg-success p-2 rounded">
            <li class="nav-item">
                <a class="nav-link active btn btn-secondary" href="checklistPT-install.php">Checklist Pullout/Install</a>
            </li>
            <li class="nav-item">
                <a class="nav-link text-light" href="PMScert.php">PMS Certificates</a>
            </li>
            <li class="nav-item">
                <a class="nav-link text-light" href="calibrationcert.php">Calibration Certificates</a>
            </li>
            <li class="nav-item">
                <a class="nav-link text-light" href="trainingcert.php">Training Certificates</a>
            </li>
            <li class="nav-item">
                <a class="nav-link text-light" href="brochures.php">Brochures</a>
            </li>
        </ul>
            ...
            <tbody>
                <?php foreach ($filteredFilesAndFolders as $item): ?>
                    <tr>
                        <td>
                            <?php
                            $itemPath = $checklistPTinstallDir . $item;
                            if (is_dir($itemPath)) {
